...
void charSubStr(char* str){ //This should return the string without the first char.
...
void insert(node ** tree, char binCode[], char letter){  
   /* node ** tree is a pointer to the pointer of node
    ...
    if (length > 0){
        ...
        charSubStr(binCode);//char* nextBinCode = charSubStr(binCode); // binCode to be passed
        ...
        if ((length != 0) && (firstChar == '0')){ // it is not a leaf, we go left
            insert(&(*tree)->left, binCode, letter); // insert left child w/ remaining binCode           
        } else if ((length != 0) && (firstChar == '1')){ // it is not a leaf, we go right
            insert(&(*tree)->right, binCode, letter); // insert right child w/ remaining binCode
        }
        return;
    } else{
...
void traverse(node ** tree, node ** root, char encMsg[]){ // go through the tree with the encMsg and print the msg.
    ...
    if (length > 0){
        ...
        if ((*tree)->leaf == 1){ // if the node is a leaf, print its value then continue with remaining code
            ...
            char* prevEncMsg = combSubStr(firstChar, encMsg);
            ...
            traverse(&(*tree)->left, root, encMsg); // traverse to left child with remaining EncMsg
        } else if (firstChar == '1'){ // if the node is not a leaf and the first char is 1, go to right child
            traverse(&(*tree)->right, root, encMsg); // traverse to right child with remaining EncMsg
        }
    } else {
        printf("%c", (*tree)->data);
    }
