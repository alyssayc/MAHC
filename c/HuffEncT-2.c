...
void insert(node** tree, char binCode[], char letter){  
   /* node ** tree is a pointer to the pointer of node
    ...
    node *new = malloc(sizeof(node)); // allocates memory for node temp
    new->left = NULL;
    new->right = NULL;
    ...
    //printf("DEBUG:  binCode: %s  firstChar: %c  nextBinMesg: %s  \n", binCode, firstChar, nextBinCode);

    if (!(*tree)){ 
        //printf("DEBUG: didn't pass a node\n");
    }
    ...
    if (strlen(binCode) == 1) {
        new->data = letter;
        new->leaf = 1;
        if (firstChar == '0'){ // put it in the left
            //printf("DEBUG: making leaf to the left %c\n", letter);
            (*tree)->left = new;
        } else if (firstChar == '1'){ // put it in the right
            //printf("DEBUG: making leaf to the right %c\n", letter);
            (*tree)->right = new;
        }
    } else {
        if (firstChar == '0'){ // it is not a leaf, we go left
            //printf("DEBUG: going left nextBinCode: %s\n", nextBinCode);
            if ( ((*tree)->left) == NULL ) {
                //printf("DEBUG: making new left node\n");
                (*tree)->left = new;
            }
            insert(&(*tree)->left, nextBinCode, letter); // insert left child w/ remaining binCode           
        } else if (firstChar == '1'){ // it is not a leaf, we go right
            //printf("DEBUG: going right nextBinCode: %s\n", nextBinCode);
            if ( ((*tree)->right) == NULL) {
                //printf("DEBUG: making new right node\n");
                (*tree)->right = new;
            }
...
int main(){
    node *root = malloc(sizeof(node));// instatiate the root node of the tree;
    root->leaf = 0;
    root->left = NULL;
    root->right = NULL;

    int N; // obtain the number of char:binCod
    scanf("%d", &N);
